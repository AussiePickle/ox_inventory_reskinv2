...
import { DragSource, InventoryType } from '../../typings/inventory';
...
const InventoryControl: React.FC = () => {
  ...
  // Drop handler for "Use"
  const [, use] = useDrop<DragSource, void, any>(() => ({
    accept: 'SLOT',
    drop: (source) => {
      if (source.inventory === InventoryType.PLAYER) {
        // Create properly typed object for onUse
        const usePayload = {
          ...source.item,
          inventoryKey: source.inventoryKey,
          inventoryType: source.inventory,
        };
        onUse(usePayload);
      }
    },
  ...
  // Drop handler for "Give"
  const [, give] = useDrop<DragSource, void, any>(() => ({
    accept: 'SLOT',
    drop: (source) => {
      if (source.inventory === InventoryType.PLAYER) {
        // Create properly typed object for onGive
        const givePayload = {
          ...source.item,
          inventoryKey: source.inventoryKey,
          inventoryType: source.inventory,
