...
export const getTargetInventory = (
  state: State,
  sourceInventoryKey: 'leftInventory' | 'rightInventory' | 'backpackInventory',
  targetInventoryKey?: 'leftInventory' | 'rightInventory' | 'backpackInventory'
): { sourceInventory: Inventory; targetInventory: Inventory; targetKey: string } => {
  const inventories: Record<'leftInventory' | 'rightInventory' | 'backpackInventory', Inventory | null> = {
    leftInventory: state.leftInventory,
    rightInventory: state.rightInventory,
    backpackInventory: state.backpackInventory || null,
  };

  const sourceInventory = inventories[sourceInventoryKey];
  if (!sourceInventory) throw new Error(`Source inventory ${sourceInventoryKey} is undefined`);

  const targetInventory = targetInventoryKey ? inventories[targetInventoryKey] || sourceInventory : sourceInventory;

  return {
    sourceInventory,
    targetInventory,
    targetKey: targetInventoryKey || sourceInventoryKey,
  };
};
